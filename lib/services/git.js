...
class Git {
  ...
  async add(branch, file) {
    try {
      ...
      await this.git.addConfig('user.email', 'github-upolad-provider@strapi.com');
      await this.git.addConfig('user.name', 'github-upload-provider');
      ...
      throw {
        ...
        values: { file: file.name, ..._.get(err, 'values', {}) },
      };
    }
